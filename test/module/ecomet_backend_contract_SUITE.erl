...
db_commit_skips_log_for_single_storage_commit(_Config)->
  with_ref(fun(Storages)->
    with_tlog_mock(fun()->
      ...
      receive
        {tlog_commit, _WriteLog, _DeleteLog} ->
...
with_tlog_mock(Fun)->
  ...
  meck:expect(ecomet_tlog, seq, fun(tlog_ref) ->
    1
  end),
  meck:expect(ecomet_tlog, commit, fun(tlog_ref, Write, Delete) ->
    self() ! {tlog_commit, Write, Delete},
    ok
  end),
