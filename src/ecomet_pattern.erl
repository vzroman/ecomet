...
inherit_fields(PatternID,ParentFields)->
  ...
  [case ChildFields of
     #{ Name := Child}->
       ...
       Other = FieldFields -- maps:keys( Child2 ),
       ParentOtherFields = parent_field_fields( PatternID, Name, Other ),
       ...
       OwnFields =
         maps:filter(fun(_N, Val)-> Val =/= none end, ecomet:read_fields(Field, Other )),
       Fields = maps:merge( ParentInheritFields, OwnFields ),
       ok = ecomet:edit_object(Field, Fields);
     _->
